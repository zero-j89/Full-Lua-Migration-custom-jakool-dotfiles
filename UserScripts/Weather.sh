...
if [ "$cacheage" -gt 1740 ] || [ ! -s "$cachedir/$cachefile" ]; then
    mapfile -t sdata < <(curl -fsS "https://wttr.in/${encoded_city}?format=%25l%0A%25C%0A%25f&lang=en" 2>/dev/null)
    ...
    if [ "${#sdata[@]}" -ge 3 ]; then
        ...
        temp_only=$(curl -fsS "https://wttr.in/${encoded_city}?format=%25f" 2>/dev/null || true)
        ...
        if [ -n "$loc" ] && [ -n "$cond_only" ] && [ -n "$temp_only" ]; then
            ...
            url="https://en.wttr.in/${encoded_city}?1&u"
