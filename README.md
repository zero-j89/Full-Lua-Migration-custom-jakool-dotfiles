
Okay so this is basically jakoolits dotfiles but slightly modified for my stack so theres a few things you will want to edit
before you use these( if you choose to ).DO NOT FORGET TO BACKUP YOUR ORIGINAL /.config/hypr folder!! For these to work you will need the original jakoolits 
dependancies. I suggest removing the whole /hypr folder entirely or
just renaming it so when you replace it with these files you wont have a confusing directory.. OKAY! Down to
basics here..This DOES NOT contain waybar(id like to help folks out there if they need so feel free to reach out)
/configs/ENVariables.lua...You want to use mine as a syntax example.. DO NOT just blindly change to my settings. 
You might want to edit all your Monitors.lua files (and monitors.lua) otherwise they should auto generate via Nwg-displays
to your monitor/s...Same will need to be done for workspaces. The stack is basically done besides future addons that you choose so you can pick and choose if you have started your conversion and are stuck on some files, let this be your bible lol. This is basically the format of the original jakools "dotfiles" so if you use them this may be useful. I also have been testing a custom Noctalia shell intergration with the files (keybinds are already set for it so you may need to change some to your current rofi launcher, etc from the current noctalia + gzml plugins/edits...) and ive been reading about all the issues from the new lua conversion for calls (had most of them myself on my test stack) so i went ahead and fixed them too,  I will be uploading the shell stuff here soon.. That more or less is just me double checking to see if im leaving in any personal API stuff that I do NOT want there for the public to use. Ill include Some images of the stack
so you can see a little of it. I really hope this finds you well and helps someone out there.A lot of these files can be implmented into your conversion without taking everything. I tried to leave the stack as simple as possible and I will be adding more as I convert more.. Remember this is mostly for people, who just started maintaining their own dots after starting from Jakools preconfig.. Im doing this in hopes to eventually give back to the community which helped me out at first getting my bearings on linux.. even thouigh my personal setup is different I wqanted to put out a functioning desktop enviorment for hyprland for newcomers that i enjoyed and did my bes tto improve upon what i felt needed...well improvment.



5/15/2026 Edit: fixed all move window/resize/etc plus drop terminal script and bind. Also fixed desktop zoom and dropdown terminal both in /scipts and in its depends. Animations are done and rainbowborders are working.

5/16/2026 Edit: Fixed rainbow borders save state for reboot(new script added) also change fixed and renamed Startup_apps in/UserConfigs to now, UserStartup_apps.. fixed corresponding terms in hyprland.lua accordingly.
 <img width="2561" height="1441" alt="Screenshot_14-May_13-51-28_9486" src="https://github.com/user-attachments/assets/22e3ece1-1803-4a05-a1d3-edda018b39c3" />
<img width="2560" height="1441" alt="Screenshot_14-May_13-50-24_18725" src="https://github.com/user-attachments/assets/fe953196-784d-4111-aa9c-3ca68d144596" />
<img width="2560" height="1441" alt="Screenshot_14-May_13-50-32_22230" src="https://github.com/user-attachments/assets/41495710-5e28-46a1-8ea6-524570b4eb98" />
<img width="2561" height="1441" alt="Screenshot_14-May_13-50-46_29093" src="https://github.com/user-attachments/assets/dc3672ec-8059-4058-96e3-140a832329e4" />
<img width="2561" height="1441" alt="Screenshot_14-May_13-19-22_28323" src="https://github.com/user-attachments/assets/7a4737db-0d81-4bf2-8294-4358a911c5a3" />
<img width="2561" height="1441" alt="Screenshot_14-May_13-19-31_12186" src="https://github.com/user-attachments/assets/fb532cc5-6add-4e58-91c9-98cb586f24c6" />
<img width="2557" height="1436" alt="Screenshot_14-May_13-19-36_28255" src="https://github.com/user-attachments/assets/a1085002-505c-4560-a398-45431bf8594f" />
<img width="2561" height="491" alt="Screenshot_14-May_15-08-25_13015" src="https://github.com/user-attachments/assets/1e58cdef-56ec-48c3-94bd-f7af07951f6e" />
